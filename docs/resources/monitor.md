...
- `url` (String) URL to monitor. Required for all types **except** `HEARTBEAT` — omit it for heartbeat monitors and use the `heartbeat_url` output instead. Tip: Write url as plain text (do not use HTML entities like `&amp;`). UptimeRobot may return HTML-escaped values; the provider normalizes them to plain text on read/import.

### Read-Only

- `heartbeat_url` (String) The heartbeat endpoint generated by UptimeRobot. **Only populated for `type = "HEARTBEAT"` monitors.** Use this URL in your application to send periodic pings.
